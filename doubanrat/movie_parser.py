...
class MovieParser(spider.Parser):
    ...
    def htm_parse(self, priority, url, keys, deep, content):
        ...
        if keys[0] == "index":
            # 获取列表页中所有的电影页面Url
            list_view=soup.find(class_='list-view')
            rating_list=list_view.find_all('li')
            for item in rating_list:
                if not item.div.div.a:
                    continue
                movie_name=item.div.div.a.string.strip().replace(' ','_')
                movie_id=''
                href=item.div.div.a.get('href')
                if href:
                    print(href.split('/')[-2])
                    movie_id=str(href.split('/')[-2])
                span_list=item.find_all('span')
                date_div=item.find(class_='date')
                movie_rating=''
                for span in span_list:
                    print(span.get('class')[0])
                    if 'rating' in span.get('class')[0]:
                        movie_rating=span['class'][0][6]
                movie_date=date_div.getText().strip()
                movie=[url,movie_name,movie_id,movie_rating,movie_date]
                save_list.append(movie)
            #div_movies = soup.find_all("a", class_="nbg", title=True)
            #detail page added into usr_list
